...
raindrop_id: 1571636473
source_domain: "lexi-lambda.github.io"
...
URL Source: https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/
...
## [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)The essence of type-driven design
...
### [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)The realm of possibility
...
`foo :: Integer -> Void`
Is it possible to implement `foo`? Trivially, the answer is _no_, as `Void` is a type that contains no values, so it’s impossible for _any_ function to produce a value of type `Void`.[1](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/#footnote-1) That example is pretty boring, but the question gets much more interesting if we choose a more realistic example:
...
### [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)Turning partial functions total
...
#### [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)Managing expectations
...
Returning `Maybe` is undoubtably convenient when we’re _implementing_`head`. However, it becomes significantly less convenient when we want to actually use it! Since `head` always has the potential to return `Nothing`, the burden falls upon its callers to handle that possibility, and sometimes that passing of the buck can be incredibly frustrating. To see why, consider the following code:
...
Unlike before, GHC accepts this definition without complaint—this definition is _total_, not partial. We can update our program to use the new implementation:
...
Note that the inverse is _not_ true: there is no way to obtain the new version of `head` from the old one. All in all, the second approach is superior on all axes.

### [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)The power of parsing
...
These two functions elegantly illustrate two different perspectives on the role of a static type system: `validateNonEmpty` obeys the typechecker well enough, but only `parseNonEmpty` takes full advantage of it. If you see why `parseNonEmpty` is preferable, you understand what I mean by the mantra “parse, don’t validate.” Still, perhaps you are skeptical of `parseNonEmpty`’s name. Is it really _parsing_ anything, or is it merely validating its input and returning a result? While the precise definition of what it means to parse or validate something is debatable, I believe `parseNonEmpty` is a bona-fide parser (albeit a particularly simple one).
...
### [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)The danger of validation
...
Unfortunately, it isn’t so simple. Ad-hoc validation leads to a phenomenon that the [language-theoretic security](http://langsec.org/) field calls _shotgun parsing_. In the 2016 paper, [The Seven Turrets of Babel: A Taxonomy of LangSec Errors and How to Expunge Them](http://langsec.org/papers/langsec-cwes-secdev2016.pdf), its authors provide the following definition:
...
## [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)Parsing, not validating, in practice
...
Now the check _cannot_ be omitted, since its result is actually necessary for the program to proceed!
...
*   **Avoid denormalized representations of data, _especially_ if it’s mutable.** Duplicating the same data in multiple places introduces a trivially representable illegal state: the places getting out of sync. Strive for a single source of truth.
...
## [](https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/)Recap, reflection, and related reading
